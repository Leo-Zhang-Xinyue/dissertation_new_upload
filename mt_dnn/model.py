...
class MTDNNModel(object):
    ...
    def update(self, batch_meta, batch_data):
        ...
        #print("*********",inputs)
        logits = self.mnetwork(*inputs,task_id = task_id,task_type = task_type)
        if batch_meta['pairwise']:
    ...
    def predict(self, batch_meta, batch_data):
        ...
        if len(inputs) == 3:
            ...
            inputs.append(None)
        #inputs.append(task_id)
        score = self.mnetwork(*inputs,task_id = task_id,task_type = task_type)

            ############################################# new ##################################
        if task_type>0:
            labels = torch.FloatTensor(batch_meta['label'])
        else:
            labels = torch.LongTensor(batch_meta['label'])
        #print('$$$$$$$$$$$$', batch_meta['label'])
        if batch_meta['pairwise']:
            labels = labels.contiguous().view(-1, batch_meta['pairwise_size'])[:, 0]
        if self.config['cuda']:
            y = Variable(labels.cuda(non_blocking=True), requires_grad=False)
            ...
            y = Variable(labels, requires_grad=False)
        if task_type > 0:
            loss = F.mse_loss(score.squeeze(), y)
        else:
            #print('############', score, y)
            loss = F.cross_entropy(score, y)
        #self.dev_loss.reset()
        self.dev_loss.update(loss.item(), score.size(0))
        ############################################# new ##################################
        # if self.config.get('weighted_on', False):
        #     if self.config['cuda']:
