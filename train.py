...
from data_utils.label_map import DATA_META, DATA_TYPE, DATA_SWAP, TASK_TYPE, generate_decoder_opt
from data_utils.log_wrapper import create_logger
from data_utils.utils import set_environment, plot ###################
...
def model_config(parser):
    ...
    parser.add_argument('--shared_size', type = int, default = 512)
...
def main():
    ...
    if os.path.exists(model_path):
        ...
        #state_dict = state_dict.to(torch.device('cuda'))
        config = state_dict['config']
        #config['attention_probs_dropout_prob'] = args.bert_dropout_p
        #config['hidden_dropout_prob'] = args.bert_dropout_p
        #print('hahahahhaaha', config)
        opt.update(config)
    else:
        logger.error('#' * 20)
    ...
    for epoch in range(0, args.epochs):
        ...
        for idx, dataset in enumerate(args.test_datasets):
         ##################################new
            prefix = dataset.split('_')[0]
            if prefix not in dev_loss:
                dev_loss[prefix] = []
                dev_met[prefix] = []
                best_dev[prefix] = float('-inf')
                bad_epochs[prefix] = 0
            ...
            if dev_data is not None:
                ...
                for key, val in dev_metrics.items():
                    ###############################################new################################
                    logger.warning("Task {0} -- epoch {1} -- Dev_loss:{2}--Dev {3}: {4:.3f}".
                                   format(dataset,  epoch, model.dev_loss.avg, key, val))
                    if key_num ==1:
                        dev_met[prefix].append(val)
                    else:
                        if val >= best_dev[prefix]:
                            best_dev[prefix] = val
                            bad_epochs[prefix] = 0
                        else:
                            bad_epochs[prefix] += 1
                    key_num += 1
                dev_loss[prefix].append(model.dev_loss.avg)

                #if epoch > 20:
                #    if model.dev_loss.avg < best_dev[prefix]:
                 #       best_dev[prefix] = model.dev_loss.avg
                  #      bad_epochs[prefix] = 0
                   # else:
                    #    bad_epochs[prefix] += 1
    ...
    #print('$$$$$$$$$$$',dev_loss)
        if min(bad_epochs.values()) >= args.patience:
            logger.info(
            'No validation set improvements observed for {:d} epochs. Early stop! best epoch {}'.
                format(args.patience,bad_epochs))
            break
    ...
    for pre in dev_loss.keys():
       ...
       # if len(dev_loss) > 1:
        #    name = 'multi'
         ...
         #   name = pre
        name = args.img_name
        plot(fig_1,ax_1,dev_loss[pre], name+'_loss', label = pre)
        plot(fig_2,ax_2,dev_met[pre],name+'_met',label =pre)
